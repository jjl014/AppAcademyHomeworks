...
def below_average_years
  ...
  Movie.select('movies.yr, COUNT(*) AS bad_movies').where(
                'score < 5').group('movies.yr').order(
                'bad_movies DESC')
end
...
def pulp_fiction_actors
  ...
  Actor.select('actors.id, actors.name').joins(:movies).where(
...
def uma_movies
  ...
  Movie.select('movies.id, movies.title, movies.yr').joins(:actors).where(
                'actors.name = \'Uma Thurman\'').order('movies.yr ASC')
